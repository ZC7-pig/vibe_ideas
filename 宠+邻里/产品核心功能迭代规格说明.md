...
以“宠物寻失与匹配”为主线，明确数据模型、页面结构、接口与云函数、交互流程与匹配规则，确保大模型能够直接据此生成可运行的微信小程序代码（基于微信云开发）。
以下为我的微信开发者工具账号的相关信息：
微信开发者AppID：wx9b6a58e47c67388f
云环境ID：cloud1-9gqreqi3bc234646

## 1. 产品概述

- 产品定位：以宠物交流发帖为主要内容创作手段，借助微信小程序获取地理位置信息，面向本地养宠用户的生活服务平台。同时在特定的宠物寻失、宠物收养等类型帖子提供特定流程的本地生活服务功能。
- 目标用户：养猫/狗人士、宠物救助者、潜在领养人。
- Demo v1 范围：
  - 基础功能：宠物交流发帖，可参考贴吧的发帖与交流评论模式。除了普通交流贴之外，还需要支持“寻宠/收养”类型的功能帖子，功能贴为满足特定业务价值会单独设计特定业务流程。
  - 普通交流帖：参照小红书发帖与交流评论模式。
  - 寻宠功能帖：寻宠/发现帖子发布、设定寻找时间/地点、设定寻找宠物品种/毛色、设定寻找宠物性别、设定寻找宠物年龄、设定寻找宠物是否有特殊需求等，此类帖子具有寻找展示实效性，例如一个月、三个月等，过期后将不再其他用户展示。因为寻宠的时效性较高，
  - 收养功能贴：与寻宠功能帖类似，但是需要设定收养/期望收养宠物的地点、期望收养宠物的品种/毛色、期望收养宠物的性别、期望收养宠物的年龄、期望收养宠物是否有特殊需求等。也设定展示实效性，例如一个月、三个月等，过期后将不再其他用户展示。
  - 基础能力：用户登录（微信授权）、个人中心、简易消息中心
- 关键价值：在常规的帖子交流中通过本地化的方式，提供寻宠/收养匹配的功能流程，提升养宠用户体验。

## 2. 主线核心功能

- 首页信息流：按时间倒序展示“寻宠”和“发现”帖子，支持搜索（标题/品种/地点）。
- 发布页：支持发布“寻宠//收养”等类型；以“标题+图片+自由文本+地点+联系方式”为主，类型特有字段按需扩展。
- 帖子详情：图片轮播、信息展示、操作按钮（联系Ta、相似提醒）。
- 寻宠/收养功能：现阶段作为一种特殊的帖子类型存在，品种关、毛色、地理位置半径与时间范围为必选项。交流模式在当前版本下与普通帖子保持一致
